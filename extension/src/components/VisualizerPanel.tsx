...
function Variables({ state, previous, source }: { state?: TraceState; previous?: TraceState; source: string }) {
  const indexNames = arrayIndexVariableNames(source);
  const entries = Object.entries(state?.variables ?? {}).filter(([name, value]) => !isStructuralObject(value) && !indexNames.has(name));
...
export function VisualizerPanel(){
  ...
  const output=s.response?.result!==undefined?s.response.result:(s.response?.stdout?.trim()||undefined); const tc=s.testcase; const finished=current?.lastEvent?.type==='PROGRAM_END' || (s.states.length>0&&s.index===s.states.length-1);
  return <div className="yv-root"><div className="yv-scroll">
    ...
    {!s.loading&&<><Section title="Variables"><Variables state={current} previous={prev} source={s.source}/></Section><Section title="Call Stack">{current?.callStack?.length?<div className="yv-stack">{current.callStack.map((f:string,i:number)=><div className="yv-frame" key={`${f}-${i}`}>{f}</div>)}</div>:<div className="yv-empty">No active method calls.</div>}</Section><Section title="Data Structures"><DataStructures state={current} source={s.source}/></Section></>}
